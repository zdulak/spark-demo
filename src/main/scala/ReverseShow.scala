...
object ReverseShow extends App {
  ...
  val solution = showDFToDF(spark.read.text("show.txt"))
  solution.show()
  ...
  def showDFToDF(df: DataFrame): DataFrame = {
    ...
    (1 until colMaxIndex).foldLeft(dfRowsAsArray) {
      case (frame, i) => frame.withColumn(s"${colNames(i - 1)}", dfRowsAsArray("value")(i))
    }
      .drop($"value")
